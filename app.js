...
const initial = {workers:[],contracts:[],payments:[],settings:{companyName:"روزنا الأردنية لاستخدام العاملين في المنازل",companyPhone:""}};
...
function renderAll(){
  ...
  renderWorkers(); renderContracts(); renderPayments(); fillWorkerSelects();
...
function renderContracts(){
 $("contractsTable").innerHTML=db.contracts.map(c=>`<tr><td>${esc(workerName(c.workerId))}</td><td>${esc(c.employer)}</td><td>${esc(c.date)}</td><td>${esc(c.salary)} د.أ</td><td>${esc(c.status)}</td><td><button class="small-btn danger" onclick="deleteContract('${c.id}')">حذف</button></td></tr>`).join("")||`<tr><td colspan="6">لا توجد عقود</td></tr>`;
}
...
function fillWorkerSelects(){
 for(const id of ["cWorker","pWorker"]){$(id).innerHTML=db.workers.map(w=>`<option value="${w.id}">${esc(w.name)} — ${esc(w.passport)}</option>`).join("")}
}
...
window.editWorker=id=>{const w=db.workers.find(x=>x.id===id); if(!w)return; $("workerId").value=w.id;$("wName").value=w.name;$("wNationality").value=w.nationality;$("wPassport").value=w.passport;$("wPhone").value=w.phone;$("wStatus").value=w.status;$("wDob").value=w.dob||"";$("wNotes").value=w.notes||"";$("workerDialogTitle").textContent="تعديل ملف العاملة";$("workerDialog").showModal()};
window.deleteWorker=id=>{if(confirm("حذف ملف العاملة؟")){db.workers=db.workers.filter(x=>x.id!==id);db.contracts=db.contracts.filter(x=>x.workerId!==id);db.payments=db.payments.filter(x=>x.workerId!==id);save()}};
$("workerForm").addEventListener("submit",e=>{e.preventDefault();const id=$("workerId").value;const data={name:$("wName").value,nationality:$("wNationality").value,passport:$("wPassport").value,phone:$("wPhone").value,status:$("wStatus").value,dob:$("wDob").value,notes:$("wNotes").value,created:today()};if(id){Object.assign(db.workers.find(x=>x.id===id),data)}else{db.workers.push({id:uid(),...data})}$("workerDialog").close();save()});

$("addContract").onclick=()=>{fillWorkerSelects();$("contractForm").reset();$("cDate").value=today();$("contractDialog").showModal()};
$("contractForm").addEventListener("submit",e=>{e.preventDefault();db.contracts.push({id:uid(),workerId:$("cWorker").value,employer:$("cEmployer").value,date:$("cDate").value,salary:$("cSalary").value,status:$("cStatus").value});$("contractDialog").close();save()});
window.deleteContract=id=>{if(confirm("حذف العقد؟")){db.contracts=db.contracts.filter(x=>x.id!==id);save()}};
...
function report(type){
 ...
 if(type==="workers"||type==="all"){html+=`<h2>العاملات</h2><table><tr><th>الاسم</th><th>الجنسية</th><th>الجواز</th><th>الحالة</th></tr>${db.workers.map(w=>`<tr><td>${esc(w.name)}</td><td>${esc(w.nationality)}</td><td>${esc(w.passport)}</td><td>${esc(w.status)}</td></tr>`).join("")}</table>`}
 if(type==="contracts"||type==="all"){html+=`<h2>العقود</h2><table><tr><th>العاملة</th><th>صاحب العمل</th><th>التاريخ</th><th>الراتب</th><th>الحالة</th></tr>${db.contracts.map(c=>`<tr><td>${esc(workerName(c.workerId))}</td><td>${esc(c.employer)}</td><td>${esc(c.date)}</td><td>${esc(c.salary)}</td><td>${esc(c.status)}</td></tr>`).join("")}</table>`}
